...
async function loadProducts() {
  const productsUrl = window.FASHION_PRODUCTS_URL || '/static/data/fashion-products.json';
  try {
    const response = await fetch(productsUrl);
    if (!response.ok) throw new Error(`Failed to load products (${response.status})`);
    const data = await response.json();
    fashionProducts = Array.isArray(data) ? data : (data.products || []);
...
function renderFashionProducts(filter = 'all') {
  ...
  const filtered = filter === 'all'  ? fashionProducts
    : filter === 'sale' ? fashionProducts.filter(p => p.tag === 'sale')
    : filter === 'new'  ? fashionProducts.filter(p => p.tag === 'new')
  ...
  grid.innerHTML = filtered.map((p, i) => {
    ...
    return `
    <div class="product-card reveal visible" style="transition-delay:${i * 0.07}s" data-id="${p.id}">
...
function openDetails(id) {
  const p = fashionProducts.find(pr => pr.id === id);
  if (!p) return;
  currentDetailId = id;
...
function sharePDProduct() {
  const p = fashionProducts.find(pr => pr.id === currentDetailId);
  if (!p) return;
  if (navigator.share) {
...
function renderRelatedInPD(category, excludeId) {
  ...
  if (!container) return;
  const related = fashionProducts.filter(p => p.category === category && p.id !== excludeId).slice(0, 5);
...
if (navSearchInput) {
  navSearchInput.addEventListener('input', e => {
    ...
    const grid = document.getElementById('productsGrid');
    const matched = fashionProducts.filter(p =>
