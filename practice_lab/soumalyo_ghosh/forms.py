...
class SellerRegistrationForm(FlaskForm):
    ...
    aadhar_number = StringField('Aadhar Number', validators=[DataRequired(), message="Aadhar must be 12 digits"]))
    gst_number = StringField('GST Number', validators=[DataRequired(), Length(min=15, max=15, message="GST must be 15 characters")])
    contact_number = StringField('Contact Number', validators=[DataRequired(), message="Enter a valid 10-digit number"]))
